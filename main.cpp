...
int main(int argc, char **argv)
{
  ...
  int device = open (devname, O_RDONLY);
>>>>>>> a6eff52711ed90367448ef92eea8b3fe3d4db270
  if(device == -1)
  {
...
>>>>>>> a6eff52711ed90367448ef92eea8b3fe3d4db270
	...
	double comdist = sqrt (
		...
		+ (yfirstcom - ylastcom) * (yfirstcom - ylastcom));
	//printf("comdist: %f\n", comdist);
	...
	double phi0 = atan2 ((vec0xfirst * vec0ylast - vec0yfirst * vec0xlast),
			     (vec0xfirst * vec0xlast + vec0yfirst * vec0ylast))
	    * radtoang;
	double phi1 = atan2 ((vec1xfirst * vec1ylast - vec1yfirst * vec1xlast),
	...
	//printf("abs(y0len) = %f\n", abs(y0len));
	//printf("abs(y1len) = %f\n", abs(y1len));
	if ((abs (x0len) > swipetolerance && abs (x1len) > swipetolerance)
	    || (abs (y0len) > swipetolerance && abs (y1len) > swipetolerance))
	{
	...
	else if ((abs (x0len) > pinchtolerance || abs (x1len) > pinchtolerance)
	    || (abs (y0len) > pinchtolerance || abs (y1len) > pinchtolerance))
	{
	  if ((((x0first - x1first) * (x0first - x1first)
	      + (y0first - y1first) * (y0first - y1first))
	      - ((x0last - x1last) * (x0last - x1last)
		  + (y0last - y1last) * (y0last - y1last))) > 0)
	  {
	    printf ("2 finger pinch IN !\n");
	    //system(commands[11]);
	    swipesuccess = 1;
	  }
		  ...
		  + (y0first - y1first) * (y0first - y1first))) > 0)
	  {
	    printf ("2 finger pinch OUT !\n");
	    //system(commands[11]);
	    swipesuccess = 1;
	  }
	}
	...
	{
	  printf ("2 finger - motion less than threshold limit!!\n");
	}

	if (comdist < comdisttolerance && swipesuccess == 0)
	{
	  printf ("2 finger motion with an angle of %i degree scaled to %i\n",
	...
	double phi1 = atan2 ((vec1xfirst * vec1ylast - vec1yfirst * vec1xlast),
			     (vec1xfirst * vec1xlast + vec1yfirst * vec1ylast))
	    * radtoang;
	double phi2 = atan2 ((vec2xfirst * vec2ylast - vec2yfirst * vec2xlast),
	...
	double phi2 = atan2 ((vec2xfirst * vec2ylast - vec2yfirst * vec2xlast),
			     (vec2xfirst * vec2xlast + vec2yfirst * vec2ylast))
	    * radtoang;
	double phi3 = atan2 ((vec3xfirst * vec3ylast - vec3yfirst * vec3xlast),
	...
	if ((abs (x0len) > swipetolerance && abs (x1len) > swipetolerance
	    && abs (x2len) > swipetolerance && abs (x3len) > swipetolerance)
	    || (abs (y0len) > swipetolerance && abs (y1len) > swipetolerance
		&& abs (y2len) > swipetolerance && abs (y3len) > swipetolerance))
	{
	  if (y0first >= (ymax - offsetbottom)
	      && y1first >= (ymax - offsetbottom)
	...
	if (comdist < comdisttolerance && swipesuccess == 0)
	{
      ...
      {
	/*calculating values for 3 finger rotations*/
	double xfirstcom = (x0first + x1first + x2first + x3first + x4first)
	    / 5;
	double yfirstcom = (y0first + y1first + y2first + y3first + y4first)
	    / 5;
	...
	double phi3 = atan2 ((vec3xfirst * vec3ylast - vec3yfirst * vec3xlast),
			     (vec3xfirst * vec3xlast + vec3yfirst * vec3ylast))
	    * radtoang;
	double phi4 = atan2 ((vec4xfirst * vec4ylast - vec4yfirst * vec4xlast),
